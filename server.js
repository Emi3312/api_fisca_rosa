...
app.post('/api/admin/clients', authenticateToken, async (req, res) => {
    const { name, usoCfdiId, formaPagoId } = req.body;
    if (!name || !usoCfdiId || !formaPagoId) return res.status(400).json({ message: 'Todos los campos son requeridos.' });
    const slug = uuidv4();
    try {
        await pool.query('INSERT INTO clients (name, slug, default_uso_cfdi_id, default_forma_pago_id) VALUES (?, ?, ?, ?)', [name, slug, usoCfdiId, formaPagoId]);
